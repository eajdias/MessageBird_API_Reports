...
def calculate_business_duration(start_dt: datetime, end_dt: datetime) -> float:
    ...
    Follows canonical formula: Raw wall-clock time, same-day only.
    """
    if not start_dt or not end_dt or start_dt >= end_dt:
        return 0.0

    # Only same-day responses are counted as per project rules
    if start_dt.date() != end_dt.date():
        return 0.0

    delta = (end_dt - start_dt).total_seconds() / 60.0
    
    # Cap at 480 minutes (8 hours) as per business rules
    if delta > 480:
        return 0.0
