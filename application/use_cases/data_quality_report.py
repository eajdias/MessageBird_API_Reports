...
class DataQualityReportUseCase:
    ...
    async def execute(self, output_dir: str, start_date: Optional[str] = None, end_date: Optional[str] = None):
        ...
        self.exporter.export_excel(
            os.path.join(report_subdir, "qualidade_dados.xlsx"),
            header, rows, "Qualidade de Dados"
        )
    ...
    def _analyze_quality(self, raw_data: List) -> List[Dict[str, Any]]:
        ...
        valid_dept = sum(1 for r in raw_data if r.dept_label and r.dept_label not in ("N/A", "None", "", "Sem Departamento"))
        metrics.append({
        ...
        valid_agent = sum(1 for r in raw_data if r.metadata.get("agent_name") and r.metadata.get("agent_name") not in ("N/A", "None", "", "OUTROS"))
        metrics.append({
        ...
        # 7. Software
        valid_software = sum(1 for r in raw_data if r.metadata.get("software") and r.metadata.get("software") != "UNKNOWN")
        metrics.append({
            "name": "Software",
            "total": total,
        ...
        # 8. Canal preenchido
        valid_channel = sum(1 for r in raw_data if r.metadata.get("channel_name") and r.metadata.get("channel_name") != "Desconhecido")
        metrics.append({
            "name": "Canal",
            "total": total,
            "valid": valid_channel,
        ...
        valid_description = sum(1 for r in raw_data if r.metadata.get("description") and str(r.metadata.get("description")).strip())
        metrics.append({
