...
class AuditoriaDemandaService:
    ...
    async def build_report(self, start_date: str, end_date: str, agent_group: str = None) -> Tuple[List[str], List[Any]]:
        ...
        business_days = 0; curr = dt_start
        while curr <= dt_end:
            if curr.weekday() < 5: business_days += 1
            curr += timedelta(days=1)
        if business_days < 1: business_days = 1
        ...
        for r in msg_rows:
            ...
            if not local_dt: continue
            h = local_dt.hour
            if r["msgs_direction"] == "sent": hours[h]["sent"] += 1
            elif r["msgs_direction"] == "received": hours[h]["received"] += 1
        ...
        for r in msg_rows:
            if r["cnvs_id"] not in seen_cids:
                ...
                if local_dt: hours[local_dt.hour]["cnvs"] += 1

        data_list = []
        for h in range(24):
            total_new = hours[h]["cnvs"]
            data_list.append([f"{h:02d}", total_new, hours[h]["received"], hours[h]["sent"], round(total_new/business_days, 2)])
